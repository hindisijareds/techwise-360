...
-- Section management compatibility migration. This block is intentionally
-- repeatable so it can be run against projects created before managed sections.
...
where profiles.role = 'student'
  ...
  and not exists (
    ...
    where current_assignment.student_id = profiles.id
...
where class_sections.school_year = '2026-2027'
  and not exists (
    select 1 from public.section_activity existing_activity
    where existing_activity.section_id = class_sections.id
      and existing_activity.action = 'created'
  );
...
using (
  status = 'active'
  and exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'student' and profiles.section_id = class_sections.id)
);
...
on public.student_section_assignments for all to authenticated
using (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'teacher' and profiles.status = 'approved'))
with check (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'teacher' and profiles.status = 'approved'));
...
on public.student_section_assignments for select to authenticated
...
on public.section_activity for all to authenticated
using (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'teacher' and profiles.status = 'approved'))
with check (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'teacher' and profiles.status = 'approved'));
